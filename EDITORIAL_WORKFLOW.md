...
## Как редактировать прямо в админке

В `/admin/` есть вкладка `Редактор`. Она показывает все текстовые поля из схемы и позволяет править их прямо в браузере. Preview обновляется сразу.

Важно: UI-редактор работает как быстрый локальный черновик. Он сохраняет правки в localStorage конкретного браузера, но сам по себе не меняет прод для всей команды. Чтобы сделать правки официальными:

1. Нажмите `Скачать CSV`, если правки нужно перенести в Google Sheets.
2. Или нажмите `Скачать JSON`, если разработчик фиксирует snapshot напрямую в Git.
3. После фиксации проверьте `/admin/`, что ошибок нет.
